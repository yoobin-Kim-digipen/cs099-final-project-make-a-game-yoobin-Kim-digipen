...
class Player
{
    constructor(_defaultImage,_rightImage,_leftImage,_leftAttackImage,_rightAttackImage,_guardRight,_guardLeft,_uiImage)
    {
    ...
    update()
    {
        ...
        this.pos.addTo(this.moveVel);
        this.pos.addTo(this.attackedVel);
    ...
    updateAnim()
    {
        ...
        {
            this.LeftAttackDeltaTime += deltaTime/1000;
            if(this.LeftAttackDeltaTime > 0.4)
            {
        ...
        {
            this.RightAttackDeltaTime += deltaTime/1000;
            if(this.RightAttackDeltaTime > 0.4)
            {
        ...
        }else if(this.animState == RIGHTGUARD)
        {
            ...
            else if(this.guardRightDeltaTime >0.5 && this.isGuard == 1)
            {
                image(this.guardRightImage[5],this.pos.x-10,this.pos.y,130,130);
                this.isParrying = 0;
                console.log(this.guardRightDeltaTime);
    ...
    checkKeyboard()
    {
        ...
        // 가만히있는상태.
        if(!keyIsPressed && this.attackCheck == 0)
        {
        ...
        if(keyIsDown(65) && this.attackCheck == 0 && (!keyIsDown(37)))
            {
        ...
        // 오른쪽키가기.
        if(keyIsDown(39) && this.attackCheck == 0)
        {
            this.animDefaultDelta = 0;
            this.moveAcc.x = 0.2;
            // if(this.pos.x > this.checkCameraX)
        ...
        //왼쪽으로가기.
        if(keyIsDown(37) && this.attackCheck == 0)
        {
            this.animDefaultDelta = 0;
            this.animState = 2;
            this.moveAcc.x = -0.2;
            // if(this.pos.x > this.checkCameraX-3)
            // {
            //     this.checkCameraX += 1;
            // }
        ...
        if(keyIsDown(65) && this.attackCheck == 0 && keyIsDown(39))
        {
        ...
        if(keyIsDown(32) && this.isJump == false)
        {
        ...
        if(keyIsDown(68) && (!keyIsDown(37)) && (!keyIsDown(39)) && this.attackCheck == 0 && this.shieldCount >0)
        {
            console.log("68")
            this.isGuard = 1;
            this.checkA = 0;
        ...
        if(keyIsDown(68) && keyIsDown(37)&& this.attackCheck == 0 && this.shieldCount >0 )
        {
            console.log("37")
            this.checkA = 1;
            this.isGuard = 1;
        ...
        // 방어하는것.
        if(keyIsDown(68) && keyIsDown(39)&& this.attackCheck == 0 && this.shieldCount >0)
        {
            console.log("39");
            this.checkA = 0;
            this.isGuard = 1;
        ...
        if(this.isJump)
        {
            ...
            if(this.jumpTime < 0.1){
                if(this.moveAcc.x > 0)
                {
                        ...
                        this.jumpVel.setAngle(285);
                        this.jumpVel.setLength(3.6);
                        
                    
                    
                }else if(this.moveAcc.x<0)
                {
                    this.jumpVel.setAngle(255);
                    this.jumpVel.setLength(3.6);
                    
                }else{
                    this.jumpVel.setLength(3.6);
                    this.jumpVel.setAngle(270);
                ...
                if(this.checkRange(this.attackLeft.x,this.attackLeft.x+60,monster[a].upLeft.x,monster[a].upRight.x) && this.checkRange(this.attackLeft.y,this.attackLeft.y+50,monster[a].upLeft.y,monster[a].downRight.y))
                    {
                        ...
                        if(this.checkA == 0 )
                        {
                            ...
                            monster[a].vel.addTo(new Vec2(1,0));
    ...
    seeUIImage(x)
    {
        if(this.life == 3)
        {
            ...
            image(this.uiImage[0],x+25,10);
            image(this.uiImage[0],x+50,10);
            image(this.uiImage[0],x+75,10);
        }else if(this.life == 2)
        {
            image(this.uiImage[0],x+25,10);
            image(this.uiImage[0],x+50,10);
